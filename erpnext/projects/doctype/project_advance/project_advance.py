...
from erpnext.projects.custom_utils import get_exchange_rate
...
class ProjectAdvance(Document):
	...
	if TYPE_CHECKING:
		...
		imprest_advance_settlement: DF.Check
		imprest_party: DF.Link | None
		imprest_patry_name: DF.Data | None
	...
	def post_journal_entry(self):
		...
		adv_gl_det = frappe.db.get_value(doctype="Account", filters=adv_gl, fieldname=["account_type"], as_dict=True)
		...
		accounts.append({
			...
			"party": self.party,
			"is_advance": "Yes" if adv_gl_det else None,
