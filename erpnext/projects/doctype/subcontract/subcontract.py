...
class Subcontract(Document):
	...
	# 		if i.is_selected and flt(i.amount):
	# 			doc = frappe.get_doc("BOQ Item", i.item)
	# 			if not doc.is_group:
	...
	def update_boq(self):
		...
		try:
			...
			for i in self.boq_item:
				if i.is_selected and flt(i.amount) and i.item:
					# Find matching BOQ Item in parent BOQ
					for boq_item in boq_doc.boq_item:
						if boq_item.name == i.item:
							if not boq_item.is_group:
								# Get current values
								current_quantity = flt(boq_item.subcontract_quantity)
								current_amount = flt(boq_item.subcontract_amount)
								
								# Calculate new values
								quantity_to_add = 0 if self.boq_type == "Milestone Based" else flt(i.quantity)
								new_quantity = current_quantity + (factor * quantity_to_add)
								new_amount = current_amount + (factor * flt(i.amount))
								new_rate = new_amount / (new_quantity if new_quantity > 0 else 1)
								
								# Update BOQ Item
								boq_item.subcontract_quantity = max(0, new_quantity)
								boq_item.subcontract_rate = max(0, new_rate)
								boq_item.subcontract_amount = max(0, new_amount)
								
								boq_doc.save(ignore_permissions=True)
								break
		except Exception as e:
			frappe.log_error(f"Error updating BOQ from Subcontract {self.name}: {str(e)}", "Subcontract.update_boq")
