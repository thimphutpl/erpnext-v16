...
def test():
	...
	client.connect()
	try:
		client.bind_transceiver(system_id='nrdcl2'.strip(), password='nrdcl@btl'.strip())
...
def update_bank_ss():
	data = frappe.db.sql("""
		...
		WHERE ss.payroll_entry = 'HR-PRUN-2026-00004'
		AND sd.salary_component = 'Financial Institution Loan'
	""", as_dict=True)

	for i in data:
		b_name = frappe.db.sql("""
			SELECT sd.bank_name 
			FROM `tabSalary Detail` sd
			INNER JOIN `tabSalary Structure` ss ON sd.parent = ss.name 
			WHERE ss.name = %s 
			AND sd.salary_component = 'Financial Institution Loan'
			AND sd.amount = %s
		""", (i['salary_structure'], i['amount']), as_dict=True)

		if not b_name:
			continue

		bank_name = b_name[0].get("bank_name")

		# ✅ Update Salary Slip child row
		frappe.db.sql("""
		...
		print(f"{i['salary_slip']} - {i['amount']} - {bank_name}")
		# break
